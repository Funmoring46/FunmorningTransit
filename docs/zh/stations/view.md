# 景区站

![景区站](https://github.com/Funmoring46/FunmorningTransit/blob/main/docs/assets/view.png?raw=true){ width="750" }

景区站（英语：View Station）是在山陆区域的快线站。

## 历史

从出口附近的美景取名，本站是其中一个在2025八月底开的。

## 图片

<figure markdown="span">
  ![入口A](https://github.com/Funmoring46/FunmorningTransit/blob/main/docs/assets/gallery/view/entrancea.png?raw=true){ width="500" }
  <figcaption>入口A</figcaption>
</figure>

<figure markdown="span">
  ![平台视图](https://github.com/Funmoring46/FunmorningTransit/blob/main/docs/assets/gallery/view/platforms.png?raw=true){ width="500" }
  <figcaption>平台视图</figcaption>
</figure>

<figure markdown="span">
  ![轨道布局](https://github.com/Funmoring46/FunmorningTransit/blob/main/docs/assets/gallery/view/tracklayout.png?raw=true){ width="500" }
  <figcaption>轨道布局</figcaption>
</figure>